...
def timed_llm_call(client, api_provider, model, prompt, role, call_id, max_tokens=4096, log_dir=None,
                   ...
                   empty_response_retries=3, reasoning_effort=None, reasoning=None):
    """
    ...
    empty_retry_count = 0  # separate counter — transient provider hiccups retry first
    ...
    while True:
        try:
            ...
            # OpenRouter-style reasoning config dict. For Kimi K2.5 via OpenRouter,
            # `{"enabled": false}` is the only reliable off-switch: `exclude=true`
            # still bills reasoning tokens, `max_tokens=0` is silently ignored.
            if reasoning:
                extra_body = api_params.setdefault("extra_body", {})
                extra_body["reasoning"] = reasoning
            # OpenRouter provider routing — model-specific ignore + soft latency
            # thresholds + price cap. Based on benchmarks (DeepInfra/Parasail/Chutes
            # consistently fast+cheap; Novita/AkashML/Together slow or priciest).
            # See research report 2026-04-21 for rationale; per-provider pricing
            # verified via /api/v1/models/<slug>/endpoints.
            if api_provider == "openrouter":
                extra_body = api_params.setdefault("extra_body", {})
                prov_cfg = extra_body.setdefault("provider", {})
                if model.startswith("moonshotai/kimi"):
                    prov_cfg.setdefault("ignore", ["ModelRun", "Venice", "Novita"])
                    prov_cfg.setdefault("order", ["throughput"])
                    prov_cfg.setdefault("max_price", {"prompt": 1.20, "completion": 6.00})
                    prov_cfg.setdefault("preferred_max_latency", {"p90": 30})
                elif model.startswith("google/gemma"):
                    prov_cfg.setdefault("ignore", ["AkashML", "Novita"])
                    prov_cfg.setdefault("order", ["throughput"])
                    prov_cfg.setdefault("max_price", {"prompt": 0.30, "completion": 0.80})
                    prov_cfg.setdefault("preferred_max_latency", {"p90": 30})
                elif model.startswith("z-ai/glm"):
                    prov_cfg.setdefault("order", ["throughput"])
                    prov_cfg.setdefault("preferred_max_latency", {"p90": 30})
                prov_cfg.setdefault("allow_fallbacks", True)
            ...
            # Malformed JSON body from provider (OpenRouter routing transient) — retry like server error.
            # JSONDecodeError's str(e) is "Expecting value: ..." without 'json' keyword, so rely on isinstance.
            is_json_decode_error = isinstance(e, json.JSONDecodeError) or (
                "expecting value" in str(e).lower() or "json" in str(e).lower()
            )
            ...
            # For empty responses, try N retries before giving up and marking incorrect
            if is_empty_response and empty_retry_count < empty_response_retries:
                empty_retry_count += 1
                sleep_time = sleep_seconds * random.uniform(0.5, 1.5)
                print(f"[{role.upper()}] Empty response — retry {empty_retry_count}/{empty_response_retries} after {sleep_time:.1f}s")
                time.sleep(sleep_time)
                continue

            # For empty responses after exhausting retries, fall back to incorrect marker
            if is_empty_response:
            ...
            if (is_timeout or is_rate_limit or is_server_error or is_json_decode_error) and attempt < retries_on_timeout:
                attempt += 1
                if is_rate_limit:
                ...
                elif is_json_decode_error:
                    error_type = "returned malformed JSON"
                    base_sleep = sleep_seconds
                else:
