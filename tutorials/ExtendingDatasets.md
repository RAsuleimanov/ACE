...
You don't need to implement these - they work automatically with any `DataProcessor` that implements the 3 required methods. Evaluation-time exceptions are recorded in `sample_failures` and counted as incorrect predictions rather than being dropped from the denominator.
