...
def log_llm_call(log_dir, call_info):
    """Append one JSON line per LLM call to <log_dir>/llm_calls.jsonl.

    Thread-safe; FH cached across calls so the lock only serializes write+flush.
    """
    global _LLM_LOG_FH, _LLM_LOG_PATH
    now = datetime.now()
    call_info['timestamp'] = now.strftime("%Y%m%d_%H%M%S_%f")[:-3]
    call_info['datetime'] = now.isoformat()
    line = json.dumps(call_info, ensure_ascii=False) + "\n"
    path = os.path.join(log_dir, "llm_calls.jsonl")

    with _LLM_LOG_LOCK:
        if _LLM_LOG_FH is None or _LLM_LOG_PATH != path:
            if _LLM_LOG_FH is not None:
                _LLM_LOG_FH.close()
            os.makedirs(log_dir, exist_ok=True)
            _LLM_LOG_FH = open(path, 'a', encoding='utf-8')
            _LLM_LOG_PATH = path
        _LLM_LOG_FH.write(line)
        _LLM_LOG_FH.flush()
