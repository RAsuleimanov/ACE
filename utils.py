...
def initialize_clients(api_provider):
    ...
    if api_provider == "sambanova":
        ...
        raise ValueError(
            f"Invalid api_provider name: {api_provider}. Must be 'sambanova', 'together', 'openai', 'commonstack', or 'openrouter'"
        )
        
    client_kwargs = dict(api_key=api_key, base_url=base_url)
    if api_provider == "openrouter":
        client_kwargs["timeout"] = openai.Timeout(connect=30.0, read=600, write=600, pool=600)
